...
- `(public)/` — marketing site: home (`page.tsx`), `/work` + `/work/[slug]` case studies, `/industries/[slug]`, `/pricing`, `/process`, `/blog` + `/blog/[slug]`, `/faq`, plus one thin route per seasonal campaign (`/fall-mini-sessions`). Mostly static/SSG with JSON-LD structured data. Shares a Nav+Footer layout.
- `(admin)/admin/` — studio OS, admin-role only. Its `layout.tsx` calls `requireAdmin()` and renders the tab bar. Subroutes: dashboard (leads + computed follow-ups), `crm`, `clients` + `clients/[id]`, `proposals` + `proposals/new`, `invoices`, `assets`, `social`, `templates`, `analytics`, `campaigns`, `projects/[id]`.
...
Each domain (`leads`, `proposals`, `clients`, `projects`, `crm`, `assets`, `social`, `pricing`, `review`, `campaigns`) owns its server actions and pure logic, imported by the route components. Key patterns:
...
- **`src/modules/campaigns/`** drives the seasonal landing pages. One layout (`CampaignPage.tsx`) renders any campaign; every string, price, inclusion, gallery slot and form option comes from a `Campaign` object in `campaigns.ts`, and the palette from a theme in `campaign.ts`. A route file is three lines (`campaignRoute(slug)`) — the "Adding a season" note at the top of `campaigns.ts` is the procedure. The studio edits the offer, price, copy, CTA and both emails from **Admin → Campaigns** without a deploy; those overrides live in the shared `diagnostic_config` store under `campaign_<slug>_*`, with the code values as fallbacks, so a blank or malformed row can never publish an empty page.
...
**Approved and built, in order:** the Content Growth Diagnostic sends a result email via Resend (`src/modules/diagnostic/email.ts`, free tier, plain `fetch` — no SDK dependency); the first-visit offer popup sends the coupon (`src/modules/offer/email.ts`); and the seasonal campaign pages send two — a confirmation to the customer and a copy of the request to the studio (`src/modules/campaigns/email.ts`, explicitly requested with the Fall Mini Sessions page). All share the one transport in `src/lib/mail.ts`. All are strictly opt-in on env (`RESEND_API_KEY` + `MAIL_FROM_EMAIL`/`DIAGNOSTIC_FROM_EMAIL`); with those unset every feature still captures its lead and simply doesn't send. Sending is always best-effort and wrapped so a delivery failure can never fail a lead capture. Do not add further outbound email without asking.
